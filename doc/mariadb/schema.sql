...
-- Sat 20 Jan 2018 11:36:43 AM CET
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `maestro`.`test_results` (`test_id` INT, `test_number` INT, `sut_id` INT, `sut_tags` INT, `sut_name` INT, `sut_version` INT, `test_result` INT, `error` INT, `env_resource_name` INT, `env_resource_role` INT, `test_rate_min` INT, `test_rate_max` INT, `test_rate_geometric_mean` INT, `test_rate_standard_deviation` INT, `test_rate_skip_count` INT, `connection_count` INT, `test_date` INT, `test_tags` INT, `test_report_link` INT);
